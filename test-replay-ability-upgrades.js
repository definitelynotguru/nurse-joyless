...
const goodAsGoldPowderSingleTurnLog = '|turn|1\n|switch|p1a: Vivillon|Vivillon, L80\n|switch|p2a: Gholdengo|Gholdengo, L80\n|move|p1a: Vivillon|Powder|p2a: Gholdengo\n|-singleturn|p2a: Gholdengo|move: Powder';
const goodAsGoldPowderSingleTurnParser = vm.runInContext(`(() => {
  const parser = new ReplayParser();
  parser.parse(${JSON.stringify(goodAsGoldPowderSingleTurnLog)});
  return parser.replayRead;
})()`, context, { timeout: 10000 });
const goodAsGoldPowderSingleTurnTarget = goodAsGoldPowderSingleTurnParser.strongest;
assert(goodAsGoldPowderSingleTurnTarget?.ruledOutAbilities?.includes('Good as Gold'), 'single-turn landed status effects should rule out Good as Gold when Powder actually connected');
assert(goodAsGoldPowderSingleTurnTarget?.notes?.some(note => /Powder successfully landed/i.test(note)), 'single-turn landed status effects should explain the successful-status contradiction in replay notes');
assert(goodAsGoldPowderSingleTurnTarget?.detectiveInputs?.[0]?.label === 'Powder landed', 'single-turn landed status effects should stay detective-loadable with the landed move name');
